...
public:
    ...
    MOCK_METHOD(ResponseStruct, makeGetRequest, (const Host& host, const std::string& target,
                                          const std::shared_ptr<Params>& params,
                                          const std::shared_ptr<Params>& headers), (override));
    MOCK_METHOD(ResponseStruct, makePostRequest, (const Host& host, const std::string& target,
                                           const std::shared_ptr<Params>& body,
                                           const std::shared_ptr<Params>& headers), (override));
    MOCK_METHOD(ResponseStruct, parseResponse, (Response response), (override));
    MOCK_METHOD(std::string, createURL, (const std::string& target, const std::shared_ptr<Params>& params),
                (override));
    MOCK_METHOD(bool, connect, (unsigned short port), (override));
    MOCK_METHOD(ResponseStruct, makeRequest, (const Host& host, const std::string& target, boost::beast::http::verb method,
                                       const std::shared_ptr<Params>& params,
                                       const std::shared_ptr<Params>& body,
                                       const std::shared_ptr<Params>& headers), (override));
};
