...
std::vector<Solution> SolutionRepository::getSolutionsBySenderId(size_t sender_id) {
    try {
        ...
        std::tuple<size_t, std::string, size_t, std::string, size_t,
                std::string, std::string, std::string, size_t, std::string> row;
        while (stream >> row) {
            solutions.emplace_back(get<0>(row), get<1>(row),
                                   get<2>(row), get<3>(row),
                                   get<4>(row), get<5>(row),
                                   get<6>(row), get<7>(row),
                                   get<8>(row), get<9>(row));
...
std::vector<Solution> SolutionRepository::getSolutionsByTaskIdAndSenderId(size_t task_id, size_t sender_id) {
    try {
        ...
        std::string sql =
                (boost::format("SELECT * FROM solutions WHERE"
                               " task_id='%s' AND sender_id='%s'") % task_id % sender_id)
                        .str();
        nontransaction n(*c);
        auto stream = stream_from::query(n, sql);
        std::vector<Solution> solutions;
...
std::vector<Solution> SolutionRepository::getSolutionsByTaskIdAndLanguage(size_t task_id, std::string lang) {
    try {
        ...
        std::string sql =
                (boost::format("SELECT * FROM solutions WHERE"
                               " task_id='%s' AND language='%s'") % task_id % lang).str();
        nontransaction n(*c);
        auto stream = stream_from::query(n, sql);
        std::vector<Solution> solutions;
        std::tuple<size_t, std::string, size_t, std::string, size_t,
...
std::optional<Solution> SolutionRepository::getOriginalSolution(size_t id) {
    try {
        ...
        std::string sql = "SELECT * FROM solutions WHERE"
                          " original_solution_id=" + std::to_string(id);
        nontransaction n(*c);
        result r(n.exec(sql));
...
size_t SolutionRepository::storeSolution(Solution solution) {
    try {
        ...
        std::string sql = (boost::format(
                "INSERT INTO solutions (send_date,sender_id, source, task_id, result, tokens, "
                "astTree, original_solution_id, language) "
                "VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s') RETURNING id; ") %
                           solution.getSendDate() % solution.getSenderId() %
                           solution.getSource() % solution.getTaskId() %
                           solution.getResult() % solution.getTokens() %
                           solution.getAstTree() % solution.getOrigSolution() %
                           solution.getLanguage()).str();
        work w(*c);
        row r = (w.exec1(sql));
        w.commit();
