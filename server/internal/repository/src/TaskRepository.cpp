...
std::vector<Task> TaskRepository::getAllTasks() {
    try {
        ...
        while (stream >> row) {
            tasks.emplace_back(get<0>(row), get<1>(row),
                               get<2>(row), get<3>(row));
        }
        stream.complete();
...
void TaskRepository::updateTask(const Task &task) {
    try {
        ...
        std::string sql = (boost::format(
                "UPDATE tasks SET description = '%s', treshold = '%s', name = '%s';") %
                           task.getDescription() % task.getTreshhold() % task.getName()).str();
        work w(*c);
        w.exec(sql);
...
size_t TaskRepository::storeTask(Task task) {
    try {
        ...
        std::string sql = (boost::format(
                "INSERT INTO tasks (description, treshold, name) "
